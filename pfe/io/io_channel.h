...
class IOChannel { public:
	uint64_t counter = 0;
	bool save = false;
	unsigned char* savechar = new unsigned char[0];
	int len;

	void send_data(const void * data, int nbyte) {
		counter +=nbyte;
		if(save){
			unsigned char* new_savechar = new unsigned char[len+nbyte];
			memcpy(new_savechar, savechar, len);
			memcpy(new_savechar, data, nbyte);
			len+=nbyte;
			delete[] savechar;
			savechar = new_savechar;
		}
	...
	void send_pt(Point_relic *A, int num_pts = 1){
		for(int i = 0; i < num_pts; ++i) {
			size_t len = 33;
			send_data(&len, 4);
	...
	void recv_pt(Point_relic *A, int num_pts = 1){
